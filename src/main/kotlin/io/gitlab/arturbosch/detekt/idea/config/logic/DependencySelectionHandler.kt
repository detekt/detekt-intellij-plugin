...
object DependencySelectionHandler {
    ...
    fun syncArtifacts(
        ...
        val groupAndArtifact = node.groupAndArtifactId
        val newState = node.isChecked

        // 2. Global Synchronization: sync all occurrences of the same artifact
        synchronize(allNodes, groupAndArtifact, newState, setNodeState)
    ...
    private fun synchronize(
        allNodes: Iterable<SyncableNode>,
        groupAndArtifact: String,
        newState: Boolean,
        setNodeState: (SyncableNode, Boolean) -> Unit,
    ) {
        for (treeNode in allNodes) {
            val matchingCoordinates = treeNode.groupAndArtifactId == groupAndArtifact
