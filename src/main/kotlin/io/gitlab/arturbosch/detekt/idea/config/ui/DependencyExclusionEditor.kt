...
internal class DependencyExclusionEditor(
    ...
    private fun uncheckExcludedNodes(node: CheckedTreeNode, excluded: Set<String>, parentIsExcluded: Boolean) {
        val isExcluded = parentIsExcluded || excluded.contains(node.groupAndArtifactId)
    ...
    private fun collectUncheckedNodes(node: CheckedTreeNode, result: MutableSet<String>) {
        if (node.isChecked) {
            ...
            result.add(node.groupAndArtifactId)
        }
    }
    ...
    private fun syncArtifacts(node: CheckedTreeNode) {
        DependencySelectionHandler.syncArtifacts(CheckedTreeNodeAdapter(node), allNodesCache) { syncable, state ->
            val adapter = syncable as CheckedTreeNodeAdapter
            dependenciesTree.setNodeState(adapter.node, state)
        }
    }
    ...
    private inner class CheckedTreeNodeAdapter(val node: CheckedTreeNode) : SyncableNode {
        ...
        override val groupId: String
            get() = (node.userObject as MavenDependencyResolver.ResolvedNode).groupId

        override val artifactId: String
            get() = (node.userObject as MavenDependencyResolver.ResolvedNode).artifactId

        override val isProvided: Boolean
            get() = (node.userObject as MavenDependencyResolver.ResolvedNode).isProvided
    ...
    companion object {
        private val CheckedTreeNode.groupAndArtifactId: String
            get() {
                val node = userObject as MavenDependencyResolver.ResolvedNode
...
private class DependenciesTree(
...
) : CheckboxTree(Renderer(), createDependencyTreeNode(root), checkboxCheckPolicy) {
    ...
    companion object {
        ...
        /**
         * Recursively creates a [CheckedTreeNode] for the given [node].
         *
         * Includes cycle detection to prevent [StackOverflowError] in case of cyclic dependencies.
         */
        private fun createDependencyTreeNode(
            ...
            if (!node.isProvided) {
                ...
                for (dependency in node.children) {
                    treeNode.add(createDependencyTreeNode(dependency, nextVisited))
                }
            }
            return treeNode
        }
